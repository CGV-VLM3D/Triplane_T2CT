"""인코더별 native 입력 텍스트 조립 — Probe 공정성의 단일 소스.

각 인코더는 **자신이 학습된 형식 그대로** 텍스트를 받는다('배포 상태의 alignment'를
...
출처 인용(각 인코더 upstream):
  ...
  CT-CLIP        : 전체 report(findings + impression) — 사용자 확인.
...
from tests.alignment_probe.cases import AlignmentCase
