...
def test_tier0_overfit_compress3d_attn():
    ...
    x = torch.randn(2, 4, *COMPRESS3D_LATENT_SHAPE)
    # Cross-attn variant needs more steps: Rodin-style 1-D mean projection in
    # TriConv carries less spatial detail than 2-D adaptive pooling, so warm-up
    # phase is slower. Model still fully overfits, just past step ~300.
    initial, final = _overfit_loop_ae(model, x, n_steps=400)
