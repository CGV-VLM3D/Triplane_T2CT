# VENDORED — DO NOT EDIT THE LOGIC.
# Source: MONAI 1.5.0  monai/networks/schedulers/rectified_flow.py
#   https://github.com/Project-MONAI/MONAI/blob/1.5.0/monai/networks/schedulers/rectified_flow.py
# Reason: Text2CT (third_party/text2ct) samples with `RFlowScheduler`, which only
#   exists in MONAI >= 1.5. This repo is pinned to MONAI 1.4 (CLAUDE.md; the Report2CT
#   adapter + frozen MAISI VAE round-trip are validated against 1.4), so we vendor the
#   single scheduler file instead of bumping the global dependency.
# ONLY CHANGE vs upstream: the two package-relative imports
#   `from .ddpm import ...` / `from .scheduler import ...`
#   are rewritten to absolute `monai.networks.schedulers.*` imports (the deps exist in 1.4).
# src/baselines/text2ct_adapter.py registers this module as
#   sys.modules["monai.networks.schedulers.rectified_flow"] so the upstream config
#   `_target_` and `from monai.networks.schedulers import RFlowScheduler` both resolve here.
...
from monai.networks.schedulers.scheduler import Scheduler
