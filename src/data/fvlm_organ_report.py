...
def normal_organ_template(organ: str) -> str:
...
def build_organ_text(
    ...
    for organ in organs:
        ...
        if not len(conc):
            conc = normal_organ_template(organ)
...
def organ_abnormal_flags(
    ...
    return {
        organ: (
            organ in organ_text
            and not organ_text[organ].startswith(normal_organ_template(organ))
        )
        for organ in organs
    }
...
def fill_missing_with_template(
    ...
    for organ in organs:
        if organ not in out:
            out[organ] = normal_organ_template(organ)
    return out
